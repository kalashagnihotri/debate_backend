...
class NotificationService:
    ...
    def create_notification(self, user, message, notification_type):
        ...
        try:
            ...
            logger.info(f"Notification created for user {user.username}: {notification_type}")
            return notification
    ...
    def notify_upcoming_debate(self, debate_session, minutes_before=60):
        ...
        try:
            ...
            logger.info(f"Upcoming debate notifications sent for session {debate_session.id}")
    ...
    def notify_session_change(self, debate_session, change_message):
        ...
        try:
            ...
            logger.info(f"Session change notifications sent for session {debate_session.id}")
    ...
    def notify_moderation_action(self, target_user, moderator, action, reason):
        ...
        """
        try:
            message = f"Moderation action by {moderator.username}: {action}. Reason: {reason}"
            ...
            logger.info(f"Moderation action notification sent to {target_user.username}")
    ...
    def _send_realtime_notification(self, user, notification):
        ...
        try:
            ...
            logger.warning("WebSocket service not available for real-time notifications")
        except Exception as e:
...
try:
    ...
    def send_upcoming_debate_notifications():
        ...
        for session in upcoming_sessions:
            minutes_until_start = int((session.scheduled_start - now).total_seconds() / 60)
