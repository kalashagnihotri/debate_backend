...
class DebateConsumer(
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.info(
                f"Debate ID: {self.debate_id}, Room: {self.room_group_name}"
            )
            ...
            if not debate_session:
                logger.error(
                    f"REJECT: Debate session {self.debate_id} not found"
                )
            ...
            logger.info(
                f"Accepting WebSocket connection for user: {user.username}"
            )
            await self.accept()
