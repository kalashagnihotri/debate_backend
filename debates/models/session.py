...
class DebateSession(TimestampedMixin, StatusMixin):
    ...
    topic = models.ForeignKey(
        DebateTopic, on_delete=models.CASCADE, related_name="sessions"
    )
    ...
    scheduled_start = models.DateTimeField(
        null=True, blank=True, help_text="Optional scheduled start time for the session"
    )
    ...
    joining_started_at = models.DateTimeField(
        null=True, blank=True, help_text="When joining window opened (5 min window)"
    )
    joining_window_end = models.DateTimeField(
        null=True, blank=True, help_text="When joining window closes"
    )
    debate_started_at = models.DateTimeField(
        null=True, blank=True, help_text="When debate chat was unlocked"
    )
    debate_end_time = models.DateTimeField(
        null=True, blank=True, help_text="When debate will/did end"
    )
    voting_started_at = models.DateTimeField(
        null=True, blank=True, help_text="When 30-second voting period started"
    )
    voting_end_time = models.DateTimeField(
        null=True, blank=True, help_text="When voting period ended"
    )
    ...
    def has_active_participants(self):
        ...
        return self.participation_set.filter(
            role="participant", user__is_active=True
        ).exists()
    ...
    def close_joining_window(self):
        ...
        self.participation_set.filter(joined_at__gt=self.joining_window_end).update(
            role="viewer"
        )
