...
class MessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Message
        fields = [
            'id', 'session', 'author', 'content', 'timestamp', 'message_type', 'image_url'
        ]
...
class ParticipationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Participation
        fields = [
            'user', 'role', 'joined_at', 'is_muted', 'warnings_count', 'messages_sent', 'has_voted'
        ]
...
class DebateSessionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DebateSession
        fields = [
            'id', 'topic', 'topic_id', 'moderator', 'scheduled_start', 'duration_minutes',
            'status', 'joining_started_at', 'joining_window_end', 'debate_started_at',
            'debate_end_time', 'voting_started_at', 'voting_end_time', 'winner_participant',
            'total_votes', 'participants', 'messages', 'created_at', 'updated_at',
            'active_participants', 'viewers', 'can_join_as_participant', 'can_join_as_viewer',
            'is_voting_active', 'participant_count', 'viewer_count', 'has_active_participants'
        ]
        read_only_fields = [
            'joining_started_at', 'joining_window_end', 'debate_started_at', 'debate_end_time',
            'voting_started_at', 'voting_end_time', 'status', 'winner_participant',
...
class ModerationActionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'session', 'moderator', 'target_user', 'target_user_id', 'action', 'reason', 'timestamp'
        ]
...
class DebateVoteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DebateVote
        fields = [
            'id', 'session', 'voter', 'voted_for', 'voted_for_id', 'best_argument_message',
            'best_argument_message_id', 'timestamp'
        ]
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserProfile
        fields = [
            'user', 'total_debates_participated', 'total_debates_won', 'total_messages_sent',
            'total_votes_cast', 'reputation_score', 'email_notifications', 'created_at', 'updated_at'
        ]
